...
int forward_chunks_to_client(ProxyRequestCtx *ctx, sockaddr_x* chunkAddresses, int numChunks, bool cdn, ...){
    ...
    for (int i = 0; i < numChunks; i++) {
        ...
        if((len = XfetchChunk(&xcache, data, VIDEO_CHUNKSIZE, XCF_BLOCK, &chunkAddresses[i], sizeof(chunkAddresses[i]))) < 0) {
        ...
        if(cdn){
			...
			// data is the manifest XML response, put them into dagCache for staging
			if(stage){
...
int main(int argc, char const *argv[])
{
	...
	if(stage){
	// create stageManagerSock to communicate with Stage Manager
	stageManagerSock = registerUnixStageManager(UNIXMANAGERSOCK);
	say("[Proxy] The current stageManagerSock is %d\n", stageManagerSock);
